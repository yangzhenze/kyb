...
public class CreateModel {
	...
	static {
		...
		PASSWORD = "密码";
		PACKAGENAME = "导入包路径";*/
		FILE_PATH = "main/java/com/api/bean";
		DATABASENAME = "kyb";
			...
			while (resultSet.next()) {
				...
				if(resultSet.getString(4).equals("PRI")){
					if(resultSet.getString(2).equals("int") || resultSet.getString(2).equals("integer") || resultSet.getString(2).equals("bigint")){
						...
						result +="import org.hibernate.annotations.GenericGenerator;\n";
						fieldStr +="    @GeneratedValue(generator = \"uuid\")\n";
						fieldStr +="    @GenericGenerator(name = \"uuid\", strategy = \"uuid\")\n";
						fieldStr +="    @Column(name=\""+resultSet.getString(1)+"\")\n";
					}
				}else{
					fieldStr +="    @Column(name=\""+resultSet.getString(1)+"\")\n";
