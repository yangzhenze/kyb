...
public class CreateModel {
    ...
    public CreateModel(DbConfig config) {
        this.URL = config.getUrl();
        this.DATABASE_NAME = config.getDataBaseName();
        this.DB_URL = "jdbc:mysql://" + this.URL + ":" + config.getPort() + "/" + this.DATABASE_NAME;
        ...
        this.PACKAGE_NAME = FILE_PATH.substring(FILE_PATH.indexOf("java") + 5).replace("/", ".");
    ...
    private String CreateEntityString(String tableName) {
        ...
        String sql = "SELECT COLUMN_NAME,DATA_TYPE,COLUMN_COMMENT,COLUMN_KEY FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '" + DATABASE_NAME + "' AND table_name='"
                + tableName + "';";
        String fieldStr = "";
        ...
        result += "/**\n";
        result += " * @author "+config.getAuthor()+"\n";
    ...
    public void CreateEntity() {
        if (getConnection()) {
            try {
                ...
                if (IS_DAO) {
                    CreateDao createDao = new CreateDao(FILE_PATH, beanNames.toString(),config.getAuthor());
                    createDao.generation();
                }

                if (IS_SERVICE) {
                    CreateService createService = new CreateService(FILE_PATH, beanNames.toString(),config.getAuthor());
